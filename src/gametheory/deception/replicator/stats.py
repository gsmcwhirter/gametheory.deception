...
from gametheory.base.statsparser import StatsParserBase
from gametheory.deception.replicator.simulations import Simulation

class StatsParser(StatsParserBase):
    
    _sender_matrix = Simulation.sender_matrix
    _receiver_matrix = Simulation.receiver_matrix
    _effective_zero = Simulation._effective_zero
    
    def _add_listeners(self):
    
        def _set_options(this):
            this._oparser.add_option("-n", "--nosumm", action="store_false", dest="summary", default=True, help="do not output summary statistics")
            this._oparser.add_option("-l", "--voistats", action="store_true", dest="voi", default=False, help="calculate value of information stats")
            this._oparser.add_option("-k", "--klstats", action="store_true", dest="kl", default=False, help="caluclate KL value stats")
            this._oparser.add_option("-r", "--rpayoffs", action="store", dest="rpoffs", choices=["simil", "dist"], default="simil", help="receiver payoff type")
            this._oparser.add_option("-s", "--spayoffs", action="store", dest="spoffs", type="int", default=0, help="sender payoff structure index")
            this._oparser.add_option("-q", "--quiet", action="store_true", dest="quiet", default=False, help="only output VoI / KL aggregates")
            
        def _check_options(this):
            if this._options.spoffs < 0 or this._options.spoffs > 2:
                this._oparser.error("Sender payoff index out of bounds")
        
        def _handle_result_options(this, out, options):
            this._result_options = options
            
        def _handle_result(this, out, count, result):
            try:
        ...
        def _when_done(this, out):
            ...
            for dup in this._duplications:
                ...
                final_sender = [(j, st) for (j, st) in enumerate(final_sender) if st >= 10. * this._effective_zero]
                final_receiver = [(j, st) for (j,st) in enumerate(final_receiver) if st >= 10. * this._effective_zero]
        
                massaged_dups.append((final_sender, final_receiver, generations))
        
            print >>out,  "Total Duplications: %i, avg generations: %.2f" % (len(massaged_dups), float(sum(i[2] for i in massaged_dups)) / float(len(massaged_dups)))
        
            if this._options.rpoffs == "simil":
                r_payoffs = p.receiver_sim_2
            else:
                r_payoffs = p.receiver_dist_2
        
            if this._options.summary:
                this._output_summary(out, massaged_dups)
        
            if this._options.voi:
                this._output_voistats(out, massaged_dups, r_payoffs)
        
            if this._options.kl:
                if this._options.spoffs == 0:
                    s_payoffs = r_payoffs[0]
                elif this._options.spoffs == 1:
                    if this._options.rpoffs == "simil":
                        s_payoffs = p.sender_sim_2_1
                    else:
                        s_payoffs = p.sender_dist_2_1
                elif this._options.spoffs == 2:
                    if this._options.rpoffs == "simil":
                        s_payoffs = p.sender_sim_2_2
                    else:
                        s_payoffs = p.sender_dist_2_2
        
                this._output_klstats(out, massaged_dups, r_payoffs[0], s_payoffs)
                
        self.on('oparser set up', _set_options)
        self.on('options parsed', _check_options)
        self.on('result', _handle_result)
        self.on('result options', _handle_result_options)
        self.on('done', _when_done)

    def _values_of_information(self, final_sender, final_receiver, r_payoffs, n=2):
        ...
        for i, msg in enumerate(msgs):
            ...
            for j, state in enumerate(states):
                if any(self._sender_matrix(s)[j][i] == 1 for (s, s_prob) in final_sender):
                    qs.append(j)
            ...
            if len(qs) == 0:
                ...
                for j, state in enumerate(states):
                    ...
                    for (r, r_prob) in final_receiver:
                        if self._receiver_matrix(r)[i][j] == 1:
                            p += r_prob
    ...
    def _kl_measures(self, final_sender, final_receiver, n=2):
        msgs = list(itertools.product(range(n), range(n)))
        states = list(itertools.product(range(n), range(n)))
        ...
        for i, msg in enumerate(msgs):
            ...
            for j, state in enumerate(states):
                ...
                for (s, sprob) in final_sender:
                    if self._sender_matrix(s)[j][i] == 1:
                        p += sprob
    ...
    def _output_summary(self, out, duplications):
        ...
        for e in ks:
            ...
            for rec, snd in itertools.izip_longest(e[0], e[1], fillvalue=False):
                print >>out,  "\t",
                if rec:
                    print >>out,  rec[0],":",rec[1],
                else:
                    print >>out,  " ",
                print >>out,  "\t\t",
                if snd:
                    print >>out,  snd[0],":",snd[1]
                else:
                    print >>out,  " "
            print >>out,  "\t\t\t\t\t(%i times, %.2f avg gen)" % (end_states[e][0], float(sum(end_states[e][1])) / float(len(end_states[e][1])))
    
        print >>out,  "=" * 72
    
    def _output_voistats(self, out, duplications, r_payoffs):
        options = self._options
        times_neg_value, times_neg_value_avg = 0, 0
    
        for (final_sender, final_receiver, generations) in duplications:
            (vois, qs) = self._values_of_information(final_sender, final_receiver, r_payoffs)
            if not options.quiet:
                print >>out,  "States sending messages (msg : states):"
                for (msg, mqs) in qs:
                    print >>out,  "\t", msg,":",mqs
                print >>out,  "Values of Information (msg : expected voi):"
                for (msg, val) in vois:
                    print >>out,  "\t", msg,":",val
                print >>out, 
        ...
        print >>out,  "Number of duplications with a negative average VoI: %i" % (times_neg_value_avg,)
        print >>out,  "=" * 72
    
    def _misinfo(self, msg_i, infos, cprobs_m_s):
    ...
    def _output_klstats(self, out, duplications, r_payoffs, s_payoffs):
        options = self._options
        ...
        for dup_i, (final_sender, final_receiver, generations) in enumerate(duplications):
            ...
            (kls, cprobs, cprobs2) = self._kl_measures(final_sender, final_receiver)
            ...
            if not options.quiet:
                ...
                for (msg, cps) in cprobs2:
                    print >>out,  "\t",msg,":",cps
                print >>out,  "KL Information Measures (msg : I_msg(state) : I(msg)):"
                for i, (msg, infos) in enumerate(kls):
                    print >>out,  "\t",msg,":",infos,":",sum(cprobs2[i][1][j] * pj for j, pj in enumerate(infos) if not math.isinf(pj))
    
            misinfo_msgs = [msg_i for msg_i, (msg, infos) in enumerate(kls) if self._misinfo(msg_i, infos, cprobs)]
        ...
        print >>out,  "Number of duplications with misinformation: %i" % (times_misinformation,)
        print >>out,  "Number of duplications with selfish senders: %i" % (times_sender_decept,)
        print >>out,  "Average sender selfishness: %.2f" % (math.fsum(avg_s_hdecept) / float(len(avg_s_hdecept)),)
        print >>out,  "Number of duplications with hurt receivers: %i" % (times_receiver_decept,)
        print >>out,  "Average receiver hurt: %.2f" % (math.fsum(avg_r_hdecept) / float(len(avg_r_hdecept)),)
        print >>out,  "Number of duplications with deception: %i" % (times_full_decept,)
        print >>out,  "Average deception probability (by sender): %.2f" % (math.fsum(avg_s_decept) / float(len(avg_s_decept)),)
        print >>out,  "Average deception probability (by receiver): %.2f" % (math.fsum(avg_r_decept) / float(len(avg_r_decept)),)
        print >>out,  "=" * 72
